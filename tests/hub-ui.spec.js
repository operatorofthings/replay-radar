...
test('signed-in hub separates areas, shows discovery reasons, and refreshes ownership explicitly',async({page})=>{
 ...
 await page.route('**/api/discover',r=>r.fulfill({json:{status:'complete',games:[{appid:2,name:'New game',match:80,reasons:[{name:'Owned game'}]}],profile:[{appid:1,name:'Owned game',hours:10,usable:true}]}}));
 ...
 await page.getByRole('button',{name:/Discover Find new games/}).click();await expect(page.locator('#discover')).toBeVisible();await expect(page.locator('#radar')).toBeHidden();await expect(page.locator('#discover')).toContainText('Because you played: Owned game');
